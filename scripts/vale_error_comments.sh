...
FILES_AND_LINES=$(git diff --unified=0 --diff-filter=M HEAD~1 HEAD "*.adoc" ':(exclude)rest_api/*' ':(exclude)modules/example-content.adoc' ':(exclude)modules/oc-adm-by-example-content.adoc' | \
grep -v -e '^[+-]' -e '^index' | \
